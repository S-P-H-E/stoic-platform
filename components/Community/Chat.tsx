  ...
  export default function Chat({currentChannel, currentUser, channelId, members, canFetch, readPermission, userId, userStatus}: {currentChannel: Channel | undefined, currentUser: Member | undefined, channelId: string | string[], members: Member[], canFetch: boolean, readPermission: boolean, userId: string | null, userStatus: string | undefined}) {
    ...
    const [prevRepliedMessage, setPrevRepliedMessage] = useState<Message | undefined>(undefined);
          ...
          {messages.map((message) => (
            ...
            >
              ...
              <div className="absolute -top-10 right-2 bg-[--border] rounded-xl p-1 flex gap-1 items-center opacity-0 group-hover:scale-y-100 scale-y-50 scale-x-[.8] group-hover:scale-x-100 group-hover:opacity-100 transition duration-300">
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger onClick={() => handleReply(message)} className="hover:bg-[--highlight] h-full p-2 rounded-lg transition duration-200"><RiReplyFill/></TooltipTrigger>
