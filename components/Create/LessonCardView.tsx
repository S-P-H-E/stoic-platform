...
export default function LessonCardView({
  ...
  return (
    <div
      className={clsx(
        'rounded-xl w-full h-60  border border-border hover:border-highlight group relative transition duration-500 overflow-hidden',
        !courseIdProp && 'hover:shadow-[0_0px_65px_rgba(49,_76,_80,_0.55)]'
      )}
    >
      <div className="absolute flex gap-2 right-5 top-5 z-30">
        ...
        <button
          onClick={handleLessonLockedChange}
          className="p-2 hover:!bg-highlight z-40 bg-darkgray border transition duration-500 border-border group-hover:bg-border group-hover:border-highlight aspect-square rounded-full"
        >
          {isLocked ? <Lock /> : <Unlock />}
        </button>

        <div className="p-2 bg-darkgray border transition duration-500 border-border group-hover:bg-border group-hover:border-highlight aspect-square rounded-full">
          {lesson.type == 'video' ? <Play /> : <Type />}
        </div>
      </div>
      <Link href={`/create/${courseId}/${lesson.id}`}>
        <div className="flex gap-8 w-full h-full p-6 z-20 pt-10">
          <div className="flex flex-col gap-2 z-20 w-full h-full justify-center p-4">
            ...
            <p className="text-highlight whitespace-pre-wrap line-clamp-2">
              {lesson.description ? lesson.description : 'Loading...'}
            </p>
            ...
            >
              <ButtonShad
                variant="secondary"
                className="text-black gap-2 active:scale-90 transition pr-8"
              >
                <FaEye />
                Watch
              </ButtonShad>
        ...
        <Image
          className="object-cover rounded-xl brightness-50 transition duration-500 scale-150 group-hover:brightness-75 group-hover:scale-[1.15]"
          alt="test"
          src={WavePattern}
          placeholder="blur"
          fill
