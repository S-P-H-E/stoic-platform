...
const UserIdHeader = ({
    ...
    return (
        <section className="w-full">
            <div className="w-full h-[36svh] md:h-[48svh] min-h-[12rem] max-h-[40rem] relative bg-darkgray">
                <div className="group relative w-full xl:w-[80%] lg:w-[85%] h-full max-w-7xl flex flex-col mx-auto lg:rounded-b-2xl overflow-hidden">
                    ...
                    {isAuthorized && (
                        <Dialog>
                            <DialogTrigger>
                                <div
                                    className="z-20 absolute w-full h-full inset-0 opacity-0 bg-bg/50 text-2xl lg:text-3xl font-medium flex items-center justify-center lg:rounded-b-2xl group-hover:opacity-100 transition">
            ...
            <div className="w-full bg-darkgray border-b border-border pb-6">
                <div className="px-6 z-20 gap-4 flex justify-between items-center xl:w-[80%] lg:w-[85%] max-w-7xl max-h-7xl mx-auto h-3/4 w-full relative">
                    <div className="absolute bottom-0 left-6 ring-8 ring-darkgray w-28 md:w-36 xl:w-52 bg-darkgray aspect-square rounded-full">
                        <div className="group relative h-full w-full rounded-full">
                            <Avatar className="w-full h-full">
                                ...
                                <AvatarFallback
                                    className='uppercase bg-white text-4xl md:text-5xl select-none w-full z-10 h-full'>{user.name && user.name.charAt(0)}{user.name && user.name.charAt(user.name.length - 1)}</AvatarFallback>
                            ...
                            {isAuthorized && (
                                <Dialog>
                                    <DialogTrigger asChild>
                                        <div
                                            className="cursor-pointer absolute w-full h-full inset-0 opacity-0 bg-bg/50 text-2xl lg:text-3xl font-medium flex items-center justify-center rounded-full group-hover:opacity-100 transition">
                    ...
                    <div className="ml-[7.5rem] md:ml-[9.5rem] xl:ml-[13.5rem] px-2 py-4">
                        <h1 className={clsx("text-2xl xl:text-3xl font-semibold line-clamp-1",
/*                            user.roles &&
                            user.roles !== 'User' &&
                            user.roles.length > 0 &&
                            user.roles[0]?.color &&
                            `text-${getUserRoleColor(user.roles)}`*/
                        )}>
