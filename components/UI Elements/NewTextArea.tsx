...
export default function NewTextArea({customHeight, className, id, label, black, ...props}: NewInputProps) {
  return (
    <div className={clsx("rounded-lg w-full min-w-0 flex", className, black ? 'bg-bg' : 'bg-darkgray')}>
        <div className="relative bg-inherit w-full">
            <textarea {...props} id={id || 'textarea'} className={clsx("disabled:opacity-50 disabled:cursor-not-allowed placeholder-transparent focus:placeholder-muted-foreground peer bg-transparent pt-2 resize-none w-full rounded-lg text-gray-200  ring-2 px-2 ring-highlight focus:ring-sky-600 focus:outline-none focus:border-rose-600", customHeight ? customHeight : 'h-16')}/>
