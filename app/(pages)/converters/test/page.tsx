    ...
    <div className="min-h-screen p-8 gap-4 flex flex-col items-center justify-center max-w-6xl w-full mx-auto">
      ...
      {videoInfo && (
        ...
        <div className="flex flex-col gap-2 h-full">
          ...
          <div className='flex gap-2 items-center w-full'>
            <div className='group w-12 h-12 relative rounded-full overflow-hidden'>
              <Image className='object-cover' fill src={videoInfo.authorThumbnailUrl} alt='Author Image'/>
              <div className="absolute top-0 left-0 w-full h-full bg-black opacity-0 group-hover:opacity-50 transition-opacity duration-200"></div>
            <Link
        ...
        <div className='flex h-52 justify-between flex-col items-center gap-2 w-full'>
          <h1 className='line-clamp-1 text-3xl font-medium'>{videoInfo.videoTitle}</h1>
          <div className='grid grid-cols-2 gap-4 w-full'>
            {videoInfo.downloadLinks.map((link, i) => (
              <Link key={i} href={link.url}>
                <Button className='gap-2 group'><div className='flex gap-2 items-center group-hover:scale-110 transition-transform'><HiDownload/>{link.quality}</div></Button>
              </Link>
            ))}
          </div>
        </div>
